...
class InvitationGuestsController extends Controller
{
            ...
            if ($guest->email != null) {
                ...
                if($user_template->locale == 'ar'){
                    $message = $data['full_name'] . "\n" .
                        "أرسل لك دعوة لحضور " . $data['title'] . "\n" .
                        "يرجى تأكيد الحضور لاستلام بطاقة الدعوة، أو الاعتذار عبر الرابط المرفق:\n" .
                        $data['url'];
                }
                ...
                $this->karix->sendInvitation((string) $guest->phone, [$name, $event_name, $date], $data['image'], [$guest_token, $user_template_token, $host_token,$user_template->locale]); //TODO
                SMSService::sendFccSMS($message, $phone);
            }
        }
    ...
    {
        $all_guests = InvitationGuests::where('status', '!=', 'pending')->where(['user_template_id' => $id])->count();
        $attending_guests = InvitationGuests::where(['user_template_id' => $id, 'is_attending' => true])->count();
        $regret_guests = InvitationGuests::where(['user_template_id' => $id, 'is_attending' => false])->count();
        $sent_guests = InvitationGuests::where(['user_template_id' => $id, 'status' => 'Sent', 'is_attending' => null])->count();
        $answerd_guests = InvitationGuests::has('guestAnswers')->where(['user_template_id' => $id, 'is_attending' => true])->count();
