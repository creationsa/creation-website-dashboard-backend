...
{
    private $model, $resource, $simple_resource, $request;
    ...
    public function __construct(string $model, string $resource = null, string $simple_resource = null, string $request = null)
    {
        $this->model    = $model;
        $this->resource = $resource;
        $this->simple_resource = $simple_resource;
    ...
    public function index()
    {
        try {
            ...
            $data = $this->simple_resource ? $this->simple_resource::collection($collections) : $collections;
    ...
    protected function applyConditionType($query, $conditionType, $whereConditions)
{
    ...
    foreach ($whereConditions as $field => $value) {
        if (
            ! (
                ($conditionType == 'whereLike' && method_exists($modelInstance, 'isTranslationAttribute') && $modelInstance->isTranslationAttribute($field)) ||
    ...
    public function store()
    {
        ...
        try {
            ...
            return $this->successResponse($data, trans('Created successfully'), 201);
        } catch (\Exception $e) {
            return $this->errorResponse([], 'An error occurred: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function update($id)
    {
        ...
        try {
            ...
            return $this->successResponse($data, trans('Updated successfully'));
        } catch (\Exception $e) {
            return $this->errorResponse([], 'An error occurred: ' . $e->getMessage(), 500);
        }
    }
    ...
    {
        $record = $this->model::findOrFail($id);
        if ($record->hasRelation) {
            return $this->errorResponse([], trans('This record has related data and cannot be deleted'), 422);
        }
        try {
            $record->delete();
            return $this->deleteResponse(trans('Deleted successfully'), 200);
        } catch (\Exception $e) {
            return $this->errorResponse([], 'An error occurred: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function toggle($id, $column)
    {
        ...
        try {
            ...
            return $this->successResponse($resource, trans('Updated successfully'));
        } catch (\Exception $e) {
    ...
    protected function deleteResponse(string $message = 'Success', int $statusCode = 200)
    {
        return response()->json([
            'data'    => null,
            'status'  => 'success',
