...
class CancelTripNotification extends Notification implements ShouldBroadcast
{
    use Queueable;
    ...
    public function __construct($trip, $type = null)
    {
        $this->trip = $trip;
        $this->type = $type;
        $this->user = $trip->plan->user;
        $this->title = [
            'ar' => trans('Your Trip has been cancelled', [], 'ar'),
            'en' => trans('Your Trip has been cancelled', [], 'en'),
        ];
        if($this->type != null && $this->type == 'auto_cancel') {
            $this->body = [
                'ar' => trans('Your Trip has been cancelled due to timeout', [], 'ar'),
                'en' => trans('Your Trip has been cancelled due to timeout', [], 'en'),
            ];
        } else {
            $this->body = [
                'ar' => trans('Your Trip has been cancelled from :name', ['name' => $this->user->full_name], 'ar'),
                'en' => trans('Your Trip has been cancelled from :name', ['name' => $this->user->full_name], 'en'),
            ];
        }
    }
